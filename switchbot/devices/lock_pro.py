...
COMMAND_LOCK_INFO = f"{COMMAND_HEADER}0f4f8102"
...
class SwitchbotLockPro(SwitchbotDevice):
    ...
    def retrieve_encryption_key(device_mac: str, username: str, password: str):
        async def async_fn():
            async with aiohttp.ClientSession() as session:
                return await SwitchbotLockPro.async_retrieve_encryption_key(
    ...
    async def async_retrieve_encryption_key(
        ...
        try:
            auth_result = await SwitchbotLockPro.api_request(
                session,
                "account",
        ...
        try:
            userinfo = await SwitchbotLockPro.api_request(
        ...
        try:
            device_info = await SwitchbotLockPro.api_request(
                session,
                f"wonderlabs.{region}",
